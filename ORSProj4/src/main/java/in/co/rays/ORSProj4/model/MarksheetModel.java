...
// get Student Name
		...
		MarksheetBean duplicateSId = findBystudentId(bean.getStudentId());
		int pk = 0;
	...
	public void delete(MarksheetBean bean) throws ApplicationException {
		...
		try {
			...
			ps.executeUpdate();
			 System.out.println("id in m model>>>>>>>"+bean.getId());
			conn.commit();
			ps.close();
		} catch (Exception e) {
    ...
    throw new DuplicateRecordException("Roll No is already exist");
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE ST_MARKSHEET SET ROLL_NO=?, STUDENT_ID=?,NAME=?,PHYSICS=?,CHEMESTRY=?,MATHS=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
...
//		}
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		System.out.println("mmmmmmmmmmmm------------"+list.size());
	...
	public List getMeritList(int pageNo, int pageSize) throws ApplicationException {
		...
		System.out.println("inside mmlist of marksheetmodel");
		ArrayList list = new ArrayList();
		StringBuffer sql = new StringBuffer(
				"SELECT `ID`,`ROLL_NO`, `NAME`, `PHYSICS`, `CHEMESTRY`, `MATHS` , (PHYSICS + CHEMESTRY + MATHS) as total from `ST_MARKSHEET` order by total desc");
		// if page size is greater than zero then apply pagination
		if (pageSize > 0) {
			// Calculate start record index
			pageNo = (pageNo - 1) * pageSize;
	...
	public MarksheetBean findBystudentId(Long sid) throws ApplicationException {
		log.debug("Model findBySID Started");

...
public abstract class BaseCtl extends HttpServlet {
	...
	/* The Constant OP_SAVE. */ //containt generic constant
...
//	public static final String OP_LOG_OUT = "Logout";
	...
	protected BaseBean populateDTO(BaseBean dto, HttpServletRequest request) {
		...
		if (userbean == null) {
			...
			if ("null".equalsIgnoreCase(createdBy) || DataValidator.isNull(createdBy)) {
				...
				//System.out.println("in base ctl create by>>>>"+createdBy);
			}
		}
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		 ...
		 System.out.println(">>>>>>>>>in service of basectl");
		// Load the preloaded data required to display at HTML form
		preload(request);

		String op = DataUtility.getString(request.getParameter("operation"));
		System.out.println("get op in base ctl service:>>>>>>>>"+op);
		...
		if (DataValidator.isNotNull(op) && !OP_CANCEL.equalsIgnoreCase(op) && !OP_VIEW.equalsIgnoreCase(op)
			...
			System.out.println("check op in basectl ");
			if (!validate(request)) {
				System.out.println("inside validate of basectl");
				BaseBean bean = (BaseBean) populateBean(request);
				// System.out.println("=======-----======)))"+bean);
				ServletUtility.setBean(bean, request); 
				System.out.println("out validate of base ctl");

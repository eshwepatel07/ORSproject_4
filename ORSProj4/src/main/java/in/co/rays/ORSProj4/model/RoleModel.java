...
public class RoleModel {
	 ...
	 private static Logger log = Logger.getLogger(RoleModel.class);
	    ...
	    public long add(RoleBean bean) throws ApplicationException,
	        ...
	        int pk=0;
	      //  int n=0;
		  RoleBean duplicataRole = findByName(bean.getName()); // Check if create Role
		 // already exist 
	 ...
	 * DuplicateRecordException { log.debug("Model update Started"); Connection conn
	 * = null; try { RoleBean duplicataRole = findByName(bean.getName()); // Check
	 * if updated Role already exist if (duplicataRole != null &&
	 * duplicataRole.getId() != bean.getId()) { throw new
	 * DuplicateRecordException("Role already exists"); }
	 * 
	 * conn = JDBCDataSource.getConnection();
	 * 
	 * conn.setAutoCommit(false); // Begin transaction PreparedStatement pstmt =
	 * conn
	 * .prepareStatement("UPDATE ST_ROLE SET NAME=?,DESCRIPTION=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?"
	 * ); pstmt.setString(1, bean.getName()); pstmt.setString(2,
	 * bean.getDescription()); pstmt.setString(3, bean.getCreatedBy());
	 * pstmt.setString(4, bean.getModifiedBy()); pstmt.setTimestamp(5,
	 * bean.getCreatedDatetime()); pstmt.setTimestamp(6,
	 * bean.getModifiedDatetime()); pstmt.setLong(7, bean.getId());
	 * pstmt.executeUpdate(); conn.commit(); // End transaction pstmt.close(); }
	 * catch (Exception e) { log.error("Database Exception..", e); try {
	 * conn.rollback(); } catch (Exception ex) { throw new ApplicationException(
	 * "Exception : Delete rollback exception " + ex.getMessage()); } throw new
	 * ApplicationException("Exception in updating Role "); } finally {
	 * JDBCDataSource.closeConnection(conn); } log.debug("Model update End"); }
	 * 
	 */	    
	    ...
	    public RoleBean findByName(String name) throws ApplicationException, RecordNotFoundException {
...
//	                "SELECT * FROM ST_ROLE WHERE NAME=?");
	  ...
	  //      StringBuffer sql = new StringBuffer("SELECT * FROM ST_ROLE WHERE ID=?");
	    ...
	    public void update(RoleBean bean) throws ApplicationException,
		  ...
		  if (duplicataRole != null && duplicataRole.getId() !=
	    ...
	    public List list(int pageNo, int pageSize) throws ApplicationException {
	        ...
	        System.out.println("rolemodel list start");
	        ArrayList list = new ArrayList();
	        ...
	        System.out.println("preload query>>: "+sql);
	        Connection conn = null;
